...
import {updateModel} from "../scenes/bbCoaching.js";
...
export class View extends Croquet.View {
    constructor(croquetModel) {
        ...
        onmousedown = e => {
            this.mouseDown(eToXY(e));
        }
        onmouseup = e => {
            this.mouseUp(eToXY(e));
        }
        onmousemove = e => {
            this.mouseMove(eToXY(e));
    ...
    initScene(info) {
        this.publish("scene", "initScene", info);
    }

    updateScene(info) {
        this.publish("scene", "updateScene", info);
    }

    event(state, pos, info) {
        this.updateScene({
            who: this.viewId,
            what: state,
            where: pos,
    ...
    mouseMove(p) {
        this.event(this.isDown ? 'drag' : 'move', p);
    }

    mouseUp(p) {
        this.isDown = false;
        this.event('release', p, this.color);
    }
}
